<template>
  <f7-page name="article">
    ...
    <template v-else>
      <f7-block strong>
        <p>
          <f7-icon size="22px" f7="calendar_today"></f7-icon>
          {{ article.date }}
        </p>
        <p v-if="article.tags && article.tags.length">
                    <span class="article-tag" v-for="tag in article.tags" :key="tag.id">
                        <f7-icon size="18px" f7="tag_fill"></f7-icon> <f7-link
                        :href="`/tag/${tag.id}/`">{{ tag.title }}</f7-link>
                    </span>
        </p>
      </f7-block>
        ...
        <f7-row>
          <f7-col>
            <f7-button @click="likeHandler(!isLiked)">
              {{ likes }}
              <f7-icon v-if="isLiked" ios="f7:hand_thumbsup_fill" aurora="f7:hand_thumbsup_fill"
                       md="f7:hand_thumbsup_fill"></f7-icon>
          ...
          <f7-col>
            <f7-button @click="dislikeHandler(!isDisliked)" color="red">
              <f7-icon v-if="isDisliked" ios="f7:hand_thumbsdown_fill" aurora="f7:hand_thumbsdown_fill"
                       md="f7:hand_thumbsdown_fill"></f7-icon>
      ...
      <f7-block strong>
        <f7-row>
          <f7-col class="social-col">
            <f7-link class="social-link-instagram" external icon-f7="logo_instagram"
                     href="https://instagram.com/gazprom.nedra"/>
          </f7-col>
          <f7-col class="social-col">
            <f7-link class="social-link-facebook" external icon-f7="logo_facebook"
                     href="https://facebook.com/geofizika2016"/>
          </f7-col>
          <f7-col class="social-col">
            <f7-link class="social-link-youtube" external
                     href="https://youtube.com/channel/UCBQqB1K9yT3hnHnS7aqs9ag#menu">
...
export default {
  ...
  methods: {
    async fetch() {
      ...
      try {
        const data = await ArticleRepository.getArticle(parseInt(this.id, 10));
    ...
    async likeHandler(value) {
      try {
        await RatingRepository.like(this.id, value);
        this.isLiked = value;
        ...
        if (value && this.isDisliked) {
          await RatingRepository.dislike(this.id, false);
    ...
    async dislikeHandler(value) {
      try {
        await RatingRepository.dislike(this.id, value);
        ...
        if (value && this.isLiked) {
          await RatingRepository.like(this.id, false);
